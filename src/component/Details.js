...
export default function Details() {
  ...
  const getImageUrl = (imgPath) => {
    ...
    if (imgPath.startsWith("storage/")) {
      return `https://ibrahim.page.gd/public/${imgPath}`;
    }
    if (imgPath.startsWith("public/")) {
      return `https://ibrahim.page.gd/public/${imgPath.slice(7)}`;
    }
    return `https://ibrahim.page.gd/public/${imgPath}`;
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // جلب البيانات
        const res = await fetch(`https://ibrahim.page.gd/api/${type}/${id}`);
        ...
        if (!localStorage.getItem(visitKey)) {
          await fetch(`https://ibrahim.page.gd/api/${type}/${id}/stats`, {
            method: "POST",
  ...
  const handleLike = async () => {
    ...
    try {
      const res = await fetch(`https://ibrahim.page.gd/api/${type}/${id}/stats`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
